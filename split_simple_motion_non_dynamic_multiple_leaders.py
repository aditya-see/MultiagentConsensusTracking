...
class SimpleFollower(Dynamics):
    ...
    def leader_selection(self, leaders, followers):
        ...
        if self.leader_check == 0:
            self.dist_lead = [np.linalg.norm(l.current_state -
                                   self.current_state) for l in leaders]

            for i in range(0, len(leaders)):
                if self.dist_lead[i] <= r:
        ...
        if len(self.neighbors_followers) > 0:
            ...
            # Check if all neighbors have same flag. If
            # not then it sets leader flag to zero
        ...
        if self.depth_flag == 1 and self.leader_flag == 0 and \
            ...
            bl = dict([(l.identity, self.dist_lead[l.identity-1]) for l in self.neighbors_leaders])
    ...
    def observed_identifier(self, leaders, followers):
        ...
        if dist_lead <= r:
            ...
            for i in range(0, len(self.neighbors_followers)):
                ...
                if  dist < dist_prev and self.identity !=\
                 self.neighbors_followers[i].identity and self.leader_flag ==\
                 self.neighbors_followers[i].leader_flag:
...
followers.append(SimpleFollower([1.2, 1.8]))
...
followers.append(SimpleFollower([.6, 2.7]))
followers.append(SimpleFollower([0.3, 3.7]))
followers.append(SimpleFollower([-0.7, 3.]))
followers.append(SimpleFollower([-1, 4.]))
followers.append(SimpleFollower([-2.2, 3.3]))
followers.append(SimpleFollower([.2, 4.6]))
followers.append(SimpleFollower([.85, 5]))
followers.append(SimpleFollower([-2.2, 4.21]))
followers.append(SimpleFollower([1.6, 4]))
followers.append(SimpleFollower([-2.3, 2.1]))

leaders = []
leaders.append(SimpleLeader([0., 0], [-40., -40]))
leaders.append(SimpleLeader([3., 2.5], [100.,100]))
leaders.append(SimpleLeader([-1., 5], [-10., 100]))
...
# for l in range(0, SimpleFollower.followers_count):
#     print("a",l,"   ", followers[l].leader_flag, followers[l].depth_flag,
#           followers[l].veh_id, "  ", followers[l].init_state)
graph = [[] for i in range(0, SimpleFollower.followers_count)]
graph_lead = [[] for i in range(0, SimpleFollower.followers_count)]
for l in range(0, SimpleFollower.followers_count):
    dist_followers = [np.linalg.norm(followers[j].current_state -
        followers[l].current_state) for j in range(0, len(followers))]
    dist_leaders = [np.linalg.norm(leaders[j].current_state -
        followers[l].current_state) for j in range(0, len(leaders))]
    for j in range(0, SimpleFollower.followers_count):
        if j != l and dist_followers[j] <= r:
            graph[l].append(followers[j].identity)
    for j in range(0, SimpleLeader.leader_count):
        if dist_leaders[j] <= r:
            graph_lead[l].append(leaders[j].identity)

    print("a",l,":",followers[l].init_state,"   ", graph[l],"  ",graph_lead[l])
